    ...
    private void initEditText() {
        ...
        addTextChangedListener(new TextWatcher() { // 对文本内容改变进行监听
            ...
            public void onTextChanged(CharSequence paramCharSequence, int paramInt1, int paramInt2, int paramInt3) {
                ...
                //add
               /* String temp = paramCharSequence.toString();
                String addChar = temp.substring(paramInt1);
                String str = phoneEt.getText().toString();
                if (temp.length() == 3 || temp.length() == 8) {
                    if (StringUtil.isEmpty(addChar)) {
                        temp += "-";
                        phoneEt.setText(temp);
                        phoneEt.setSelection(temp.length());
                    }
                }*/
            }
        });
